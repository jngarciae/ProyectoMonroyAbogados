		...
		_onSearchFieldSearch: function(oEvent) {
			...
			return new Promise(function(fnResolve) {
				...
				if (sQuery && sQuery.length > 0) {
					...
					aFilters.push(new sap.ui.model.Filter("PhoneNumer", sap.ui.model.FilterOperator.Contains, sQuery));

					aFilters.push(new sap.ui.model.Filter("Email", sap.ui.model.FilterOperator.Contains, sQuery));

					aFilters.push(new sap.ui.model.Filter("Website", sap.ui.model.FilterOperator.Contains, sQuery));

					aFilters.push(new sap.ui.model.Filter("CompanySize", sap.ui.model.FilterOperator.Contains, sQuery));

					aFilters.push(new sap.ui.model.Filter("CompanyLogo", sap.ui.model.FilterOperator.Contains, sQuery));

					aFilters.push(new sap.ui.model.Filter("PrimaryContact", sap.ui.model.FilterOperator.Contains, sQuery));

					aFilters.push(new sap.ui.model.Filter("SecondContact", sap.ui.model.FilterOperator.Contains, sQuery));
			...
			if (status) {
				...
				return new Promise(function(fnResolve, fnReject) {
					...
					if (bHasPendingChanges) {
						...
						var oNewEntityInstance = Utilities.getDefaultValuesForDetailPage2();

						oModel = this.getView().getModel();
						var oNewBindingContext = oModel.createEntry("CompanySet", {
							properties: oNewEntityInstance
						});

						this.doNavigate("DetailPage2", oNewBindingContext, fnResolve);
					}
				}.bind(this)).catch(function(err) {
					if (err !== undefined) {
						MessageBox.error(err.message);
